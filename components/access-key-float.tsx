...
export function AccessKeyFloat() {
  ...
  function end() { if (!dragging.current) return; dragging.current = false; const width = window.innerWidth < 640 ? 128 : 154; const gap = 8; const x = position.x + width / 2 < window.innerWidth / 2 ? gap : Math.max(gap, window.innerWidth - width - gap); const y = Math.max(56, Math.min(window.innerHeight - 120, position.y)); const snapped = { x, y }; setPosition(snapped); window.localStorage.setItem("buildbox-access-key-position", JSON.stringify(snapped)); }
