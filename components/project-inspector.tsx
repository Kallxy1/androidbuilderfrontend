...
export function ProjectInspector({ file, onFile }: Props) {
  ...
  return <div>
    <label className="dropzone flex cursor-pointer flex-col items-center justify-center rounded-2xl p-8 text-center">
      ...
      <div className="mb-3 flex size-12 items-center justify-center rounded-2xl bg-muted">{reading ? <FileArchive className="size-6 animate-pulse" /> : <UploadCloud className="size-6" />}</div>
      <div className="font-semibold">{file ? file.name : "Upload project ZIP"}</div>
      <div className="mt-1 text-xs text-muted-foreground">Maksimal 100 MB · struktur project akan diperiksa otomatis</div>
    </label>
    {report && <div className="mt-3 space-y-2 rounded-xl border border-border bg-muted p-4 text-xs">
      <div className="font-bold">Project inspection</div>
