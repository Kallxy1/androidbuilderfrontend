...
export function BuildProgress({ status, currentStep, message, startedAt }: Props) {
  ...
  const phase = active.includes("download") || active.includes("prepar") ? 1 : active.includes("setup") || active.includes("install") ? 2 : active.includes("detect") ? 3 : active.includes("build") || active.includes("gradle") || active.includes("compile") ? 4 : active.includes("upload") ? 6 : status === "success" ? 6 : 1;
  const percent = status === "success" ? 100 : status === "failure" ? 100 : Math.min(94, Math.max(8, Math.round((phase / 6) * 100)));
